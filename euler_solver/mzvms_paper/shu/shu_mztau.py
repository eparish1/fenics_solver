...
element = VectorElement("CG", mesh.ufl_cell(), p,dim=6)
V = FunctionSpace(mesh, element)


# Define boundary condition
#u0L = Constant([1.0,0.0,2.5,0,0,0]) #sod
u0L = Constant([3.857143,10.141852232767,39.1666692650425,0,0,0]) #sod

bcl = DirichletBC(V, u0L, boundary_left)
#u0R = Constant([0.125,0.0,0.25,0,0,0]) #sod
u0R = Constant([0.9475250292592142,0.,2.5,0,0,0])
...
#R_strong = evalInviscidFluxLinNS(U,Ux)
Rortho = [None]*3
...
for i in range(0,3):
  Rortho[i] = R_strong[i] - U[i+3]

Rlin = evalInviscidFluxLinNS(U,Rortho) 
...
#       inner(JTPhi[i],tau*Rortho[i] )*dx 

for i in range(0,3):
  F = F + inner(Phi[i], dti*(U[i] - U_n[i]) )*dx

for i in range(3,6):
  F = F + inner(Phi[i] , U[i] )*dx - inner(Phi[i], R_strong[i-3])*dx
