...
#GUx1 = containerMatVec(GC[0][0],U.dx(0)) +  containerMatVec(GC[0][1],U.dx(1)) + containerMatVec(GC[0][2],U.dx(2))
#GUx2 = containerMatVec(GC[1][0],U.dx(0)) +  containerMatVec(GC[1][1],U.dx(1)) + containerMatVec(GC[1][2],U.dx(2))
#GUx3 = containerMatVec(GC[2][0],U.dx(0)) +  containerMatVec(GC[2][1],U.dx(1)) + containerMatVec(GC[2][2],U.dx(2))
...
for i in range(0,4):
  F +=  inner(GTX[i] + GTY[i] + GTZ[i], tau*R_strongDT[i])*dx

#for i in range(1,5):
#  F +=   inner(Phi[i].dx(0), GUx1[i] )*dx
#  F +=   inner(Phi[i].dx(1), GUx2[i] )*dx 
#  F +=   inner(Phi[i].dx(2), GUx3[i] )*dx


for i in range(0,5):
  F = F + inner(Phi[i], dti*(U[i] - U_n[i]) )*dx

sol_file = 'Solution_p3_dt' + str(dt) 
